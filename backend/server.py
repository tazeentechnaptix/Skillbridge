...
# -------------------------------
# Optional deps (flagged availability)
# -------------------------------
...
# Optional: pdfminer fallback for better PDF extraction
try:
...
# -------------------------------
# spaCy (NER) — used to enrich job keywords
# -------------------------------
try:
    nlp = spacy.load("en_core_web_sm")
    SPACY_READY = True
except Exception as e:
    print(f"[spaCy] Could not load en_core_web_sm: {e}")
    SPACY_READY = False
...
class GeminiLLM:
    ...
    def _call(self, prompt: str, temperature: float, max_output_tokens: int):
        print(f"[GeminiLLM] Calling model with prompt length {len(prompt)}")
    ...
    def generate_text(self, prompt: str, temperature: float = 0.6, max_output_tokens: int = 2048) -> str:
        ...
        if text:
            return text
        # Retry once with a briefer constraint
        try:
...
# -------------------------------
# Embeddings helper (Gemini text-embedding-004)
# -------------------------------
...
def embed_texts_with_gemini(texts: List[str]) -> List[List[float]]:
    if not GEMINI_AVAILABLE or not api_key:
        raise EmbeddingUnavailableError("Gemini embeddings not available (package or API key missing).")
...
def ner_keywords(s: str) -> Set[str]:
    if not SPACY_READY or not s:
        return set()
    doc = nlp(s)
    # Keep short but useful entity tokens
    ents = {e.text.lower() for e in doc.ents if len(e.text) > 2}
    return ents
...
def extract_keywords_from_job_text(s: str) -> Set[str]:
    ...
    tokens = tokenize(s_norm)
    base = {t for t in tokens if (t in DEFAULT_HINTS or (len(t) > 3 and any(c.isalpha() for c in t)))}
    return base | ner_keywords(s)
...
def semantic_similarity(resume_text: str, job_description: str) -> float:
    resume_vec = embed_texts_with_gemini([resume_text])[0]
    job_desc_vec = embed_texts_with_gemini([job_description])[0]
...
def smart_chunk(text: str, max_chars: int = 1200, overlap: int = 150) -> List[str]:
    ...
    while start < n:
        ...
        nice_start = min(start + int(max_chars * 0.6), window_end)
...
def extract_text_from_uploadfile(f: UploadFile) -> str:
    ...
    if name.endswith(".pdf") or "pdf" in ctype:
        ...
        raise HTTPException(500, "PDF parsing failed. Install pdfminer.six for better support.")
...
# -------------------------------
# FastAPI + CORS + LLM init
# -------------------------------
app = FastAPI(title="SkillBridge API (single-file)", version="1.2.0")
...
def require_embeddings_ready():
    if not GEMINI_AVAILABLE or not api_key:
        raise HTTPException(503, "Embeddings unavailable (Gemini package or API key missing).")
...
class SemanticSimRequest(BaseModel):
    a: str
    b: str

class SemanticSimResponse(BaseModel):
    score: float
...
def health():
    return {
        ...
        "gemini_pkg": GEMINI_AVAILABLE,
        "chroma_pkg": CHROMA_AVAILABLE,
...
# -------------------------------
# 1) Resume ↔ Job matching (with optional AI reasoning)
# -------------------------------
...
def match_resume_to_job(payload: ResumeJobMatchRequest):
    ...
    for title in (payload.aspiring_job_titles or [])[:25]:
...
def match_resume_to_courses(payload: ResumeCoursesMatchRequest):
    ...
    if len(payload.course_titles) > 200:
        raise HTTPException(400, "course_titles too many (<=200).")
...
def generate_cover_letter(payload: CoverLetterRequest):
    ...
    top_k = max(1, min(payload.top_k or 8, 20))
...
# -------------------------------
# 6) Semantic Similarity (optional utility)
# -------------------------------
@app.post("/similarity", response_model=SemanticSimResponse)
def similarity(payload: SemanticSimRequest):
    require_embeddings_ready()
    score = semantic_similarity(payload.a, payload.b)
    return SemanticSimResponse(score=score)
